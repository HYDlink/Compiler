...
let rec string_of_exp =
    ...
    | IfExp (e1, e2, e3) ->
        ...
        ^ (string_of_exp e1)
          ^ " then "
            ^ (string_of_exp e2) ^ " else " ^ (string_of_exp e3)
    | MonOpAppExp (m, e) ->
        (string_of_mon_op m)
        ^ " " ^ (paren_string_of_exp e)
    | BinOpAppExp (b, e1, e2) ->
        match b with
        | CommaOp ->
            ("("
             ^ (paren_string_of_exp e1)
               ^ (string_of_bin_op b)
            ...
            ((paren_string_of_exp e1)
             ^ " "
               ^ (string_of_bin_op b)
                 ^ " " ^ (paren_string_of_exp e2))
    | AppExp (e1, e2) ->
        (non_app_paren_string_of_exp e1)
        ^ " " ^ (paren_string_of_exp e2)
...
and paren_string_of_exp e =
    match e with
    | VarExp _
    | ConstExp _ -> string_of_exp e
    | _ -> "(" ^ string_of_exp e ^ ")"

and non_app_paren_string_of_exp e =
    match e with
    | AppExp (_, _) -> string_of_exp e
...
let rec mergesort list =
    ...
    | ([]
    | [ _ ]) as l -> l
    | l -> let left, right = split l in merge (mergesort left) (mergesort right)
...
let rec string_of_exp_cps ext_cps =
    match ext_cps with
    | VarCPS (k, x) -> paren_string_of_cps_cont k ^ " " ^ x
    | ConstCPS (k, c) ->
        paren_string_of_cps_cont k
        ^ " " ^ string_of_const c
    | MonOpAppCPS (k, m, r) ->
        paren_string_of_cps_cont k
        ^ "(" ^ string_of_mon_op m ^ " " ^ r ^ ")"
    | BinOpAppCPS (k, b, r, s) ->
        paren_string_of_cps_cont k
        ^ "(" ^ r ^ " " ^ string_of_bin_op b ^ " " ^ s ^ ")"
    | IfCPS (b, e1, e2) ->
        "IF "
        ^ b
          ^ " THEN "
            ^ string_of_exp_cps e1
              ^ " ELSE " ^ string_of_exp_cps e2
    | AppCPS (k, r, s) ->
        "("
        ^ r
          ^ " " ^ s ^ " " ^ paren_string_of_cps_cont k ^ ")"
    | FunCPS (k, x, Kvar, e) ->
        (paren_string_of_cps_cont k)
        ^ " (" ^ (string_of_funk x e) ^ ")"
    | FixCPS (k, f, x, Kvar, e) ->
    ...
    let int_to_int_26_list n =
        let rec aux n l =
            if n <= 0 then
                ...
                let c = ((n - 1) mod 26) in aux ((n - (c + 1)) / 26) (c :: l) in aux n [] in

    let rec aux l =
        match l with
        | [] -> ""
        | n :: ns -> (String.make 1 (Char.chr (n + 97))) ^ aux ns in

    aux (int_to_int_26_list n)

let freshFor lst =
    let rec fresh_ n =
        if List.mem (int_to_string n) lst then
            fresh_ (n + 1)
        else
            int_to_string n in fresh_ 1
...
let rec exp_cps_norm_aux var_subst free_vars exp_cps =
    ...
    | VarCPS (kappa, var_name) ->
        let kappa' =
            cps_cont_norm_aux var_subst free_vars kappa in VarCPS(kappa', var_subst var_name)
    | ConstCPS (kappa, Const) ->
        let kappa' =
            cps_cont_norm_aux var_subst free_vars kappa in ConstCPS(kappa', Const)
    | MonOpAppCPS (kappa, binop, argvar) ->
        let kappa' =
            cps_cont_norm_aux var_subst free_vars kappa in MonOpAppCPS(kappa', binop, var_subst argvar)
    | BinOpAppCPS (kappa, binop, fstvar, sndvar) ->
        let kappa' =
            cps_cont_norm_aux var_subst free_vars kappa in

        BinOpAppCPS(kappa', binop, var_subst fstvar, var_subst sndvar)
    | IfCPS (boolvar, thenexpcps, elseexpcps) ->
        let thenexpcps' =
            exp_cps_norm_aux var_subst free_vars thenexpcps in

        let elseexpcps' =
            exp_cps_norm_aux var_subst free_vars elseexpcps in

        IfCPS(var_subst boolvar, thenexpcps', elseexpcps')
    | AppCPS (kappa, funvar, argvar) ->
        let kappa' =
            cps_cont_norm_aux var_subst free_vars kappa in AppCPS(kappa', var_subst funvar, var_subst argvar)
    | FunCPS (kappa, x, k, bodyexpcps) ->
        ...
        let bodyexpcps' =
            exp_cps_norm_aux new_var_subst (x :: free_vars) bodyexpcps in
    ...
    | FixCPS (kappa, f, x, k, bodyexpcps) ->
        ...
        let new_var_subst name =
            if name = x || name = f then
                name
            else
                var_subst name in

        let bodyexpcps' =
            exp_cps_norm_aux new_var_subst (f :: x :: free_vars) bodyexpcps in
...
and cps_cont_norm_aux var_subst free_vars kappa =
    ...
    | FnContCPS (y, bodyexpcps) ->
        let newy = freshFor free_vars in
        ...
        let bodyexpcps' =
            exp_cps_norm_aux new_var_subst (newy :: free_vars) bodyexpcps in

        FnContCPS(newy, bodyexpcps')
